...
\paragraph{Individual games (L3).}
For each individual game of chess, a new child of the L2 subnet is created (\Cref{sec:create}).
...
When the game finishes, its result is automatically reported to the L2 subnet (\Cref{sec:cross-net-tx}), which updates the players' ratings accordingly,
and the L3 subnet is disposed of (\Cref{sec:remove}).

\paragraph{Player accounts.}
Each player has an account on the L2 subnet where they deposit funds (\Cref{sec:deposit}) from the rootnet by submitting a corresponding L1 transaction%
...
A player can transfer funds back to their L1 account through a withdraw operation (\Cref{sec:withdraw}) -- again, by submitting an L2 transaction.
