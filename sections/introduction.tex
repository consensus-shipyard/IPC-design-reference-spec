...
All components of a subnet and all users using it are assumed to fully trust their parent \akosh{Maybe worth hinting at why we can assume such a thing (e.g. running a full node on the parent); at least I think some readers might start thinking about adversarial scenarios to break this assumption.} and regard it as the ultimate source of truth.
...
\Cref{sec:example-use-case} introduces an sample use case of \ipc that we use throughout as a running example.
...
We discuss participants' incentives to spend the resources required for inter-subnet interaction in \Cref{sec:incentives} and related work in \Cref{sec:related-work}.
